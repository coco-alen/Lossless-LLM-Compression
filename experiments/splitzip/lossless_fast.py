...
Strategy: Two-kernel encode.
  ...
  Kernel 2: Triton kernel that scans the packed output for nibble==15,
            collects (position, raw_exponent) pairs into a small escape buffer.
            This kernel reads pk + exponents, writes ~3000 entries. Very fast.

The escape collection kernel is NOT like PyTorch nonzero() — it processes
the already-computed packed nibbles and only looks for the escape sentinel (15).
"""
...
# Scans packed nibbles. For each nibble==15, records (element_position, raw_exponent).
# Uses atomic counter for output positioning.

@triton.jit
def _collect_escapes(pk, inp, enc_lut,
                     esc_pos, esc_val, esc_count,
                     n_pairs, n, BLOCK: tl.constexpr):
    """Collect escaped elements (nibble index == 15) into compact arrays."""
    pid = tl.program_id(0)
...
class FastLosslessCodec:
    ...
    def calibrate(self, sample):
        ...
        for i in range(min(15, vals.numel())):
            self.enc_lut[vals[si[i]].item()] = i
            self.dec_lut[i] = vals[si[i]].item()
