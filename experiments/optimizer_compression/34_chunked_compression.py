"""
Experiment 34: Chunked Byte3 Compression

Fix exp33's peak memory issue by:
1. Processing compress/decompress in chunks (e.g. 64M elements)
2. On first step, gather+compress one state at a time (not both)
3. Aggressive intermediate deletion

Target: -285 MB savings, ~1.5x slowdown, NO peak memory spike.
"""
...
CHUNK = 64 * 1024 * 1024  # 64M elements per chunk


class CompressedAdamW:
    """Lossless FP32 optimizer state compression via byte3 6-bit packing.

    Chunked processing to avoid peak memory spikes.
    """
    ...
    @staticmethod
    @torch.no_grad()
    def _compress_chunk(fp32_chunk):
        """Compress a chunk of FP32 → (byte012, packed3, codebook, n)."""
        n = fp32_chunk.numel()
        device = fp32_chunk.device
        int32 = fp32_chunk.view(torch.int32)
    ...
    def _compress(flat_fp32):
        """Compress flat FP32 via chunked processing."""
        n = flat_fp32.numel()
        if n <= CHUNK:
            return CompressedAdamW._compress_chunk(flat_fp32)

        # Process in chunks, collecting results
        all_byte012 = []
        all_packed3 = []
        # For chunked: use global codebook (compute first, then compress)
        device = flat_fp32.device
        int32 = flat_fp32.view(torch.int32)

        # Build global codebook from byte3 of entire tensor (uint8 = small)
        byte3_all = ((int32 >> 24) & 0xFF).to(torch.uint8)
        counts = torch.bincount(byte3_all.to(torch.int32), minlength=256)
        present = (counts > 0).nonzero(as_tuple=True)[0]
        codebook = present.to(torch.uint8)
        n_unique = len(codebook)
        lut = torch.zeros(256, dtype=torch.uint8, device=device)
        ...
        for start in range(0, n, CHUNK):
            ...
            chunk_n = end - start
            chunk_int32 = int32[start:end]

            # byte3 → indices for this chunk
            chunk_byte3 = byte3_all[start:end]
            indices = lut[chunk_byte3.to(torch.int32)]
            ...
            # byte012
            byte012 = chunk_int32.view(torch.uint8).reshape(chunk_n, 4)[:, :3].contiguous().reshape(-1)

            all_byte012.append(byte012)
            all_packed3.append(packed3)

        del byte3_all, lut

        byte012 = torch.cat(all_byte012)
        del all_byte012
        packed3 = torch.cat(all_packed3)
        del all_packed3
    ...
    def _decompress(data):
        """Decompress (byte012, packed3, codebook, n) → flat FP32, chunked."""
        ...
        for start in range(0, n, CHUNK):
            ...
            chunk_b012 = byte012[b012_offset:b012_offset + chunk_n * 3]
            b012_offset += chunk_n * 3

            n_groups = (chunk_n + 3) // 4
            chunk_p3 = packed3[p3_offset:p3_offset + n_groups * 3]
            p3_offset += n_groups * 3

            result[start:end] = CompressedAdamW._decompress_chunk(
                chunk_b012, chunk_p3, codebook, chunk_n)
    ...
    def _compress_states(self):
        if self._first_step:
            self._init_params()
            # Gather and compress one state at a time to minimize peak
            self._flat_m = self._gather_to_flat('exp_avg')
            self._m_data = self._compress(self._flat_m)
            del self._flat_m; self._flat_m = None

            self._flat_v = self._gather_to_flat('exp_avg_sq')
...
def benchmark(model_name="Qwen/Qwen3-0.6B", batch_size=4, seq_len=256):
    print("\n" + "=" * 80)
    print("Lossless FP32 Optimizer Compression (Byte3 6-bit, Chunked)")
    print("=" * 80)
